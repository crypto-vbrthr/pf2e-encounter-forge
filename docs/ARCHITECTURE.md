...
Condition values may come from the normalized event payload, persistent Encounter context, or a specifically referenced participant. Persistent context includes current Combat round/turn, current phase, one explicitly referenced objective, one explicitly referenced tactical group, and encounter/group participant-state counts. Participant-context conditions carry their own Blueprint participant reference so separate conditions in one OR/AND expression can inspect different creatures. Live participant HP is resolved from the deployed Token/Actor through `ParticipantService`; it is deliberately not duplicated into Encounter Instance persistence. When a Blueprint participant expands to several Runtime participants, a single participant-context condition uses ANY-member semantics.
